    ...
    <div class="p-6 space-y-6">
        ...
        <div class="rounded-lg border bg-white shadow-sm">
            <div class="p-6 border-b">
                <h2 class="text-2xl font-bold text-gray-800">Lead Sources</h2>
                <p class="text-sm text-muted-foreground mt-1">Define where your leads originate from</p>
            </div>
            <div class="p-6">
                <form method="POST" action="{{ route('lead.source.store') }}" class="space-y-4">
                    @csrf
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div class="space-y-2">
                            <label class="text-sm font-medium leading-none" for="src-name">Source Name *</label>
                            <input id="src-name" name="name" type="text" placeholder="e.g. Social Media, Google Ads" required
                                value="{{ old('name') }}" class="{{ $input }}">
                        </div>
                        <div class="space-y-2">
                            <label class="text-sm font-medium leading-none" for="src-notes">Notes</label>
                            <input id="src-notes" name="notes" type="text" placeholder="Optional description"
        ...
        <div class="rounded-lg border bg-white shadow-sm">
            <div class="p-6 border-b">
                <h3 class="text-lg font-semibold text-gray-800">All Sources</h3>
                <p class="text-sm text-muted-foreground">{{ $sources->count() }} {{ Str::plural('entry', $sources->count()) }}</p>
            </div>

            @if ($sources->isEmpty())
                <div class="p-10 text-center text-muted-foreground text-sm">
                    <i class="fa-solid fa-globe text-3xl mb-3 text-gray-300"></i>
                    <p>No sources added yet. Use the form above to add your first one.</p>
                </div>
            @else
                <div class="divide-y">
                        ...
                        <div x-data="{ editing: false }" class="p-4">
                            <div x-show="!editing" class="flex items-start justify-between gap-4">
                                <div class="flex-1 min-w-0">
                                    ...
                                    @if ($source->notes)
                                        <p class="text-sm text-muted-foreground mt-0.5">{{ $source->notes }}</p>
                                    @endif
                                </div>
                                <div class="flex items-center gap-2 shrink-0">
                                    ...
                                    <form action="{{ route('lead.source.destroy', $source->id) }}" method="POST"
                                        onsubmit="return confirm('Delete \'{{ addslashes($source->name) }}\'?')">
                                        @csrf @method('DELETE')
                            ...
                            <form x-show="editing" action="{{ route('lead.source.update', $source->id) }}"
                                method="POST" class="space-y-3" x-cloak>
                                @csrf @method('PUT')
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                                    <div class="space-y-1">
                                        <label class="text-xs font-medium text-gray-600">Source Name *</label>
                                        <input type="text" name="name" value="{{ $source->name }}" required class="{{ $input }}">
                                    </div>
                                    <div class="space-y-1">
                                        <label class="text-xs font-medium text-gray-600">Notes</label>
                                        <input type="text" name="notes" value="{{ $source->notes }}" class="{{ $input }}">
                                    </div>
                                </div>
