    ...
    <div class="p-6 space-y-6">
        ...
        <div class="rounded-lg border bg-white shadow-sm">
            <div class="p-6 border-b">
                <h2 class="text-2xl font-bold text-gray-800">Lead Ratings</h2>
                <p class="text-sm text-muted-foreground mt-1">Define priority levels for your leads</p>
            </div>
            <div class="p-6">
                <form method="POST" action="{{ route('lead.rating.store') }}" class="space-y-4">
                    @csrf
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div class="space-y-2">
                            <label class="text-sm font-medium leading-none" for="rat-name">Rating Name *</label>
                            <input id="rat-name" name="name" type="text" placeholder="e.g. High, Medium, Low" required
                                value="{{ old('name') }}" class="{{ $input }}">
                        </div>
                        <div class="space-y-2">
                            <label class="text-sm font-medium leading-none" for="rat-notes">Notes</label>
                            <input id="rat-notes" name="notes" type="text" placeholder="Optional description"
        ...
        <div class="rounded-lg border bg-white shadow-sm">
            <div class="p-6 border-b">
                <h3 class="text-lg font-semibold text-gray-800">All Ratings</h3>
                <p class="text-sm text-muted-foreground">{{ $ratings->count() }} {{ Str::plural('entry', $ratings->count()) }}</p>
            </div>

            @if ($ratings->isEmpty())
                <div class="p-10 text-center text-muted-foreground text-sm">
                    <i class="fa-solid fa-star text-3xl mb-3 text-gray-300"></i>
                    <p>No ratings added yet. Use the form above to add your first one.</p>
                </div>
            @else
                <div class="divide-y">
                        ...
                        <div x-data="{ editing: false }" class="p-4">
                            <div x-show="!editing" class="flex items-start justify-between gap-4">
                                <div class="flex-1 min-w-0">
                                    <p class="font-medium text-gray-800">{{ $rating->name }}</p>
                                    @if ($rating->notes)
                                        <p class="text-sm text-muted-foreground mt-0.5">{{ $rating->notes }}</p>
                                    @endif
                                </div>
                                <div class="flex items-center gap-2 shrink-0">
                                    ...
                                    <form action="{{ route('lead.rating.destroy', $rating->id) }}" method="POST"
                                        onsubmit="return confirm('Delete \'{{ addslashes($rating->name) }}\'?')">
                                        @csrf @method('DELETE')
                            ...
                            <form x-show="editing" action="{{ route('lead.rating.update', $rating->id) }}"
                                method="POST" class="space-y-3" x-cloak>
                                @csrf @method('PUT')
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                                    <div class="space-y-1">
                                        <label class="text-xs font-medium text-gray-600">Rating Name *</label>
                                        <input type="text" name="name" value="{{ $rating->name }}" required class="{{ $input }}">
                                    </div>
                                    <div class="space-y-1">
                                        <label class="text-xs font-medium text-gray-600">Notes</label>
                                        <input type="text" name="notes" value="{{ $rating->notes }}" class="{{ $input }}">
                                    </div>
                                </div>
